...
## CPU-only Usage
This fork is patched for CPU-only compatibility. All `.cuda()` calls are replaced with `.to('cpu')`.
To run on CPU, install PyTorch without CUDA:
  pip install torch torchvision
Note: Sampling is much slower on CPU. For distributed/multi-GPU, see original instructions.
