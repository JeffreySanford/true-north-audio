...
def download(remote_path, local_path, async_download=False):
    # Try wget, fallback to Python-native download
    try:
        ...
        print(f"wget failed or not found: {e}. Falling back to urllib.")
    # Python-native download
    print(f"Downloading {remote_path} to {local_path} using urllib...")
    try:
        with urllib.request.urlopen(remote_path) as response, open(local_path, 'wb') as out_file:
            out_file.write(response.read())
    except Exception as e:
        print(f"Python-native download failed: {e}")
